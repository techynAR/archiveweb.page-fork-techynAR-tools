...
import { captureScript } from "./page-state/capture";
...
class Recorder {
  ...
  async initOpts() {
    ...
    this.archivePageState = (await getLocalOption("archivePageState")) === "1";
  ...
  async detach() {
    ...
    try {
      if (this.archivePageState) {
        // @ts-expect-error - TS2339 - Property 'pageInfo' does not exist on type 'Recorder'.
        if (this.pageInfo) {
          // @ts-expect-error - TS2339 - Property 'pageInfo' does not exist on type 'Recorder'.
          await this.capturePageState(this.pageInfo);
          // @ts-expect-error - TS2339 - Property 'pageInfo' does not exist on type 'Recorder'.
          await this.saveScreenshot(this.pageInfo);
        }
      }
    } catch(e) {
      console.error("Error capturing QC state:", e);
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async capturePageState(pageInfo: any) {
    const { result } = await this.pageEval(
      "__awp_capture_state",
      captureScript,
      // @ts-expect-error - TS2339 - Property 'sessions' does not exist on type 'Recorder'.
      this.sessions || []
    );

    if (result && result.type === "string") {
      try {
        const statePayload = JSON.parse(result.value);
        const finalState = {
          version: 1,
          ...
          snapshots: [
            {
              ...
              domState: statePayload,
              storage: statePayload.storage || { localStorage: {}, sessionStorage: {} },
              captureErrors: []
            }
          ]
        };

        const payloadStr = JSON.stringify(finalState);
        const payloadBytes = new TextEncoder().encode(payloadStr);

        const fullData = {
          url: "urn:page-state:" + pageInfo.url,
          ts: new Date().getTime(),
          status: 200,
          statusText: "OK",
          pageId: pageInfo.id,
        ...
        console.error("Failed to parse or save QC state", e);
      }
    }
  }
