...
class ArchiveWebApp extends ReplayWebApp {
  ...
  archivePageState: boolean | null = null;
        ...
        >
          ${this.settingsTab === "prefs"
            ? html`<fieldset>
                ...
                <div class="field is-size-6 mt-4">
                  <input
                    name="prefArchivePageState"
                    id="archivePageState"
                    class="checkbox"
                    type="checkbox"
                    ?checked="${this.archivePageState}"
                    @change=${this.onUpdatePrefsOption}
                  /><span class="ml-1">Capture final QC page state (Visual State, SVGs, Canvases)</span>
                  <p class="is-size-7 mt-1">
                    Generates a visual and DOM snapshot right before recording stops.
                    Useful for archiving annotation tasks and dynamic single-page applications.
                  </p>
                </div>
              </fieldset>`
            : ``}
